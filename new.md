...
4. Create .claude/project-info.md with CRITICAL CONTEXT SECTION template:
   ...
   **You MUST verify these facts before making ANY code changes:**
   ...
   - **Database**: [TO BE FILLED - check .env DB_CONNECTION] ⚠️ UPDATE AFTER .ENV SETUP
     (MySQL, PostgreSQL, or SQLite - must match .env DB_CONNECTION value)
...
5. Create .claude/conventions.md with TALL stack best practices template AND version placeholders:
   ...
   **This project uses Filament [VERSION TO BE DETERMINED]**
   ...
   ## CRITICAL: Filament Project Rules (Apply from Day 1)
   ...
   **Do NOT use Flux components for public-facing/frontend pages**
   ...
   - Flux is included as a Filament dependency
   - Flux components are ONLY for admin panel/Filament resources
   - For public/frontend pages: Use standard Blade components + Tailwind CSS
   - Never use Flux outside of Filament admin context
